#  Attaque 12 — Abus de trust inter-domaine (Child  Parent)
...
>  **Point technique important :** le realm client doit être `north.sevenkingdoms.local` (là où vit le ticket forgé), pas `sevenkingdoms.local`. Kerberos fait alors le **renvoi cross-realm** via le trust. Se tromper de realm  `KDC_ERR_C_PRINCIPAL_UNKNOWN`.
...
## 5.  Détection dans Wazuh —  anomalie

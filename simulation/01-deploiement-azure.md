#  GOAD sur Azure (VM Linux + VirtualBox)
...
##  Règle d'or : ÉTEINDRE la VM après chaque session
...
3. **Paramètres de base :**
   ...
   | Architecture VM | x64 |
   | **Taille** |  **Standard_E4s_v3** (4 vCPU, 32 Go) — cliquer *"Voir toutes les tailles"* pour la trouver |
...
```
> Cette fois, **environnement Linux natif**  pas d'Hyper-V, WinRM stable  le déploiement doit **aller au bout** (vagrant + ansible). ⏱ ~1 h.
